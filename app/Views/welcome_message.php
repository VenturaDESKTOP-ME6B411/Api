<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <title>Edison Ventura!</title>
    <meta name="description" content="The small framework with powerful features">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="shortcut icon" type="image/png" href="/favicon.ico" />

    <!-- STYLES -->

    <style {csp-style-nonce}>
    /* Establece una transición suave para el cambio de color de fondo y color de texto */
    * {
        transition: background-color 300ms ease, color 300ms ease;
    }

    /* Establece el estilo para el estado de enfoque (focus) de los elementos */
    *:focus {
        background-color: rgba(221, 72, 20, .2);
        outline: none;
    }

    /* Estilos generales para el HTML y el cuerpo del documento */
    html,
    body {
        color: rgba(33, 37, 41, 1);
        font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji";
        font-size: 16px;
        margin: 0;
        padding: 0;
        -webkit-font-smoothing: antialiased;
        -moz-osx-font-smoothing: grayscale;
        text-rendering: optimizeLegibility;
    }

    /* Estilos para el encabezado */
    header {
        background-color: rgba(247, 248, 249, 1);
        padding: .4rem 0 0;
    }

    /* Estilos para la barra de menú */
    .menu {
        padding: .4rem 2rem;
    }

    /* Estilos para la lista no ordenada dentro del encabezado */
    header ul {
        border-bottom: 1px solid rgba(242, 242, 242, 1);
        list-style-type: none;
        margin: 0;
        overflow: hidden;
        padding: 0;
        text-align: right;
    }

    /* Estilos para elementos de la lista en el encabezado */
    header li {
        display: inline-block;
    }

    /* Estilos para enlaces dentro del encabezado */
    header li a {
        border-radius: 5px;
        color: rgba(0, 0, 0, .5);
        display: block;
        height: 44px;
        text-decoration: none;
    }

    /* Estilos específicos para elementos de menú en el encabezado */
    header li.menu-item a {
        border-radius: 5px;
        margin: 5px 0;
        height: 38px;
        line-height: 36px;
        padding: .4rem .65rem;
        text-align: center;
    }

    /* Estilos para el enfoque y desplazamiento sobre los elementos de menú */
    header li.menu-item a:hover,
    header li.menu-item a:focus {
        background-color: rgba(221, 72, 20, .2);
        color: rgba(221, 72, 20, 1);
    }

    /* Estilos para el logotipo en el encabezado */
    header .logo {
        float: left;
        height: 44px;
        padding: .4rem .5rem;
    }

    /* Estilos para el botón de alternar menú en el encabezado */
    header .menu-toggle {
        display: none;
        float: right;
        font-size: 2rem;
        font-weight: bold;
    }

    /* Estilos para el botón dentro del menú de alternar */
    header .menu-toggle button {
        background-color: rgba(221, 72, 20, .6);
        border: none;
        border-radius: 3px;
        color: rgba(255, 255, 255, 1);
        cursor: pointer;
        font: inherit;
        font-size: 1.3rem;
        height: 36px;
        padding: 0;
        margin: 11px 0;
        overflow: visible;
        width: 40px;
    }

    /* Estilos para el enfoque y desplazamiento sobre el botón de alternar menú */
    header .menu-toggle button:hover,
    header .menu-toggle button:focus {
        background-color: rgba(221, 72, 20, .8);
        color: rgba(255, 255, 255, .8);
    }

    /* Estilos para la sección de héroe en el encabezado */
    header .heroe {
        margin: 0 auto;
        max-width: 1100px;
        padding: 1rem 1.75rem 1.75rem 1.75rem;
    }

    /* Estilos para los encabezados h1 y h2 dentro de la sección de héroe */
    header .heroe h1 {
        font-size: 2.5rem;
        font-weight: 500;
    }

    header .heroe h2 {
        font-size: 1.5rem;
        font-weight: 300;
    }

    /* Estilos generales para las secciones del contenido */
    section {
        margin: 0 auto;
        max-width: 1100px;
        padding: 2.5rem 1.75rem 3.5rem 1.75rem;
    }

    /* Estilos para el encabezado h1 dentro de las secciones */
    section h1 {
        margin-bottom: 2.5rem;
    }

    /* Estilos para los encabezados h2 dentro de las secciones */
    section h2 {
        font-size: 120%;
        line-height: 2.5rem;
        padding-top: 1.5rem;
    }

    /* Estilos para el bloque de código dentro de las secciones */
    section pre {
        background-color: rgba(247, 248, 249, 1);
        border: 1px solid rgba(242, 242, 242, 1);
        display: block;
        font-size: .9rem;
        margin: 2rem 0;
        padding: 1rem 1.5rem;
        white-space: pre-wrap;
        word-break: break-all;
    }

    /* Estilos para el código dentro de las secciones */
    section code {
        display: block;
    }

    /* Estilos para los enlaces dentro de las secciones */
    section a {
        color: rgba(221, 72, 20, 1);
    }

    /* Estilos para las imágenes SVG dentro de las secciones */
    section svg {
        margin-bottom: -5px;
        margin-right: 5px;
        width: 25px;
    }

    /* Estilos para la sección adicional con clase "further" */
    .further {
        background-color: rgba(247, 248, 249, 1);
        border-bottom: 1px solid rgba(242, 242, 242, 1);
        border-top: 1px solid rgba(242, 242, 242, 1);
    }

    /* Ajuste para el primer h2 dentro de la sección "further" */
    .further h2:first-of-type {
        padding-top: 0;
    }

    /* Estilos para el pie de página */
    footer {
        background-color: rgba(221, 72, 20, .8);
        text-align: center;
    }

    /* Estilos para la sección "environment" dentro del pie de página */
    footer .environment {
        color: rgba(255, 255, 255, 1);
        padding: 2rem 1.75rem;
    }

    /* Estilos para la sección "copyrights" dentro del pie de página */
    footer .copyrights {
        background-color: rgba(62, 62, 62, 1);
        color: rgba(200, 200, 200, 1);
        padding: .25rem 1.75rem;
    }

    /* Estilos de medios para pantallas con un ancho máximo de 629px */
    @media (max-width: 629px) {

        /* Ajuste de la lista no ordenada en el encabezado */
        header ul {
            padding: 0;
        }

        /* Ajuste del botón de alternar menú en el encabezado */
        header .menu-toggle {
            padding: 0 1rem;
        }

        /* Estilos para los elementos de menú en pantallas pequeñas */
        header .menu-item {
            background-color: rgba(244, 245, 246, 1);
            border-top: 1px solid rgba(242, 242, 242, 1);
            margin: 0 15px;
            width: calc(100% - 30px);
        }

        /* Mostrar el botón de alternar menú en pantallas pequeñas */
        header .menu-toggle {
            display: block;
        }

        /* Ocultar los elementos con la clase "hidden" en pantallas pequeñas */
        header .hidden {
            display: none;
        }

        /* Estilos para los elementos de menú al pasar el ratón o enfocar en pantallas pequeñas */
        header li.menu-item a {
            background-color: rgba(221, 72, 20, .1);
        }

        header li.menu-item a:hover,
        header li.menu-item a:focus {
            background-color: rgba(221, 72, 20, .7);
            color: rgba(255, 255, 255, .8);
        }
    }
    </style>

    </style>
</head>

<body>

    <!-- HEADER: MENU + HEROE SECTION -->
    <header>

        <div class="menu">
            <ul>
                <li class="logo"><a href="https://codeigniter.com" target="_blank"><img height="44"
                            title="CodeIgniter Logo" alt="Visit CodeIgniter.com official website!"
                            src="data:image/png;base64,"></a>
                </li>
                <li class="menu-toggle">
                    <button onclick="toggleMenu();">&#9776;</button>
                </li>
                <li class="menu-item hidden"><a href="#">Home</a></li>
                <li class="menu-item hidden"><a href="https://codeigniter4.github.io/userguide/"
                        target="_blank">Docs</a>
                </li>
                <li class="menu-item hidden"><a href="https://forum.codeigniter.com/" target="_blank">Community</a></li>
                <li class="menu-item hidden"><a
                        href="https://github.com/codeigniter4/CodeIgniter4/blob/develop/CONTRIBUTING.md"
                        target="_blank">Contribute</a>
                </li>
            </ul>
        </div>

        <div class="heroe">

            <h1>Desarrollo de Aplicaion Web <?= CodeIgniter\CodeIgniter::CI_VERSION ?></h1>

            <h2>Tarea unidad 4 actividad 7 que se vasa en un Crud </h2>

        </div>

    </header>

    <!-- CONTENT -->

    <section>

        <h1>Acerca del curso de Desarrollo de Aplicaiones Web</h1>
        <input type="text" id="valor" value="">
        <button id="boton_consulta">Consultar</button>

        <p>CRUD en CodeIgniter 3 se refiere a la implementación de las operaciones básicas (Create, Read, Update,
            Delete) en una aplicación web utilizando el framework CodeIgniter en su versión 3. Estas operaciones están
            destinadas a gestionar la información almacenada en una base de datos.</p>
        <p>Funciones:

            Create (Crear):

            Utilizar la clase de modelo y el método insert() para agregar nuevos registros a la base de datos.Read
            (Leer):

            Utilizar el método get() para recuperar datos de la base de datos. Delete (Eliminar):

            Utilizar el método delete() para eliminar registros de la base de datos. Ventajas:

            Facilidad de Desarrollo: CodeIgniter 3 simplifica el desarrollo web al proporcionar una estructura
            organizada y herramientas eficientes para realizar operaciones CRUD.

            Rapidez en el Desarrollo: La simplicidad del framework permite un desarrollo más rápido de aplicaciones web,
            especialmente aquellas centradas en operaciones CRUD.

            Documentación Abundante: CodeIgniter 3 tiene una documentación completa y amigable que facilita a los
            desarrolladores aprender y utilizar sus características.

            Desventajas:

            Versionamiento: CodeIgniter 3 ya no recibe actualizaciones ni soporte oficial. Esto significa que no se
            beneficia de las últimas características y correcciones de seguridad proporcionadas en versiones más
            recientes de CodeIgniter.

            Menos Características que Versiones Posteriores: Comparado con CodeIgniter 4, la versión 3 tiene menos
            características y herramientas avanzadas, lo que puede limitar la capacidad de desarrollo en proyectos más
            complejos.

            Compatibilidad Futura: Dado que CodeIgniter 3 ha llegado al final de su vida útil oficial, puede ser más
            difícil garantizar la compatibilidad futura con nuevas versiones de PHP u otros cambios tecnológicos.

            Es importante considerar estos factores al elegir un framework para el desarrollo de aplicaciones web y
            evaluar si las características y el soporte proporcionados por CodeIgniter 3 son suficientes para los
            requisitos del proyecto.</p>

        <p>Si desea editar esta página, la encontrará ubicada en:</p>

        <pre><code>app/Views/welcome_message.php</code></pre>

        <p>El controlador correspondiente a esta página se puede encontrar en:</p>

        <pre><code>app/Controllers/Home.php</code></pre>

    </section>

    <div class="further">

        <section>

            <h1>La further</h1>

            <h2>
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'>
                    <rect x='32' y='96' width='64' height='368' rx='16' ry='16'
                        style='fill:none;stroke:#000;stroke-linejoin:round;stroke-width:32px' />
                    <line x1='112' y1='224' x2='240' y2='224'
                        style='fill:none;stroke:#000;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px' />
                    <line x1='112' y1='400' x2='240' y2='400'
                        style='fill:none;stroke:#000;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px' />
                    <rect x='112' y='160' width='128' height='304' rx='16' ry='16'
                        style='fill:none;stroke:#000;stroke-linejoin:round;stroke-width:32px' />
                    <rect x='256' y='48' width='96' height='416' rx='16' ry='16'
                        style='fill:none;stroke:#000;stroke-linejoin:round;stroke-width:32px' />
                    <path
                        d='M422.46,96.11l-40.4,4.25c-11.12,1.17-19.18,11.57-17.93,23.1l34.92,321.59c1.26,11.53,11.37,20,22.49,18.84l40.4-4.25c11.12-1.17,19.18-11.57,17.93-23.1L445,115C443.69,103.42,433.58,94.94,422.46,96.11Z'
                        style='fill:none;stroke:#000;stroke-linejoin:round;stroke-width:32px' />
                </svg>
                Learn
            </h2>

            <p>The User Guide contains an introduction, tutorial, a number of "how to"
                guides, and then reference documentation for the components that make up
                the framework. Check the <a href="https://codeigniter4.github.io/userguide" target="_blank">User
                    Guide</a> !</p>

            <h2>
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'>
                    <path
                        d='M431,320.6c-1-3.6,1.2-8.6,3.3-12.2a33.68,33.68,0,0,1,2.1-3.1A162,162,0,0,0,464,215c.3-92.2-77.5-167-173.7-167C206.4,48,136.4,105.1,120,180.9a160.7,160.7,0,0,0-3.7,34.2c0,92.3,74.8,169.1,171,169.1,15.3,0,35.9-4.6,47.2-7.7s22.5-7.2,25.4-8.3a26.44,26.44,0,0,1,9.3-1.7,26,26,0,0,1,10.1,2L436,388.6a13.52,13.52,0,0,0,3.9,1,8,8,0,0,0,8-8,12.85,12.85,0,0,0-.5-2.7Z'
                        style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px' />
                    <path
                        d='M66.46,232a146.23,146.23,0,0,0,6.39,152.67c2.31,3.49,3.61,6.19,3.21,8s-11.93,61.87-11.93,61.87a8,8,0,0,0,2.71,7.68A8.17,8.17,0,0,0,72,464a7.26,7.26,0,0,0,2.91-.6l56.21-22a15.7,15.7,0,0,1,12,.2c18.94,7.38,39.88,12,60.83,12A159.21,159.21,0,0,0,284,432.11'
                        style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px' />
                </svg>
                Discuss
            </h2>

            <p>CodeIgniter is a community-developed open source project, with several
                venues for the community members to gather and exchange ideas. View all
                the threads on <a href="https://forum.codeigniter.com/" target="_blank">CodeIgniter's forum</a>, or <a
                    href="https://codeigniterchat.slack.com/" target="_blank">chat on Slack</a> !</p>

            <h2>
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'>
                    <line x1='176' y1='48' x2='336' y2='48'
                        style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px' />
                    <line x1='118' y1='304' x2='394' y2='304'
                        style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px' />
                    <path
                        d='M208,48v93.48a64.09,64.09,0,0,1-9.88,34.18L73.21,373.49C48.4,412.78,76.63,464,123.08,464H388.92c46.45,0,74.68-51.22,49.87-90.51L313.87,175.66A64.09,64.09,0,0,1,304,141.48V48'
                        style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px' />
                </svg>
                Contribute
            </h2>

            <p>CodeIgniter is a community driven project and accepts contributions
                of code and documentation from the community. Why not
                <a href="https://codeigniter.com/en/contribute" target="_blank">
                    join us</a> ?
            </p>

        </section>

    </div>

    <!-- FOOTER: DEBUG INFO + COPYRIGHTS -->

    <footer>
        <div class="environment">

            <p>Page rendered in {elapsed_time} seconds</p>

            <p>Environment: <?= ENVIRONMENT ?></p>

        </div>

        <div class="copyrights">

            <p>&copy; <?= date('Y') ?> CodeIgniter Foundation. CodeIgniter is open source project released under the MIT
                open source licence.</p>

        </div>

    </footer>

    <!-- SCRIPTS -->

    <script src="https://code.jquery.com/jquery-2.2.4.min.js" integrity="sha256-BbhdlvQf/xTY9gja0Dq3HiwQF8LaCRTXxZKRutelT44=
        " crossorigin="anonymous"></script>

    <script>
    function toggleMenu() {
        var menuItems = document.getElementsByClassName('menu-item');
        for (var i = 0; i < menuItems.length; i++) {
            var menuItem = menuItems[i];
            menuItem.classList.toggle("hidden");
        }
    }
    $("boton_consulta").click(function() {

        $.get("test2/local", function(data, status) {
            alert("hola" + data[0].nombres + " " + data[0].apellido1 + " " + data[0].apellidos2);

        })

    })





    console.log("Hola esto es la consola prueba 26");
    </script>

    <!-- -->

</body>


</html>